...
//
//   Field           Type              Size
//   txhash          common.Uint256    32 bytes
...
// values.
func dbPutBlockIDIndexEntry(dbTx database.Tx, hash *common.Uint256, id uint32) error {
...
func dbRemoveBlockIDIndexEntry(dbTx database.Tx, hash *common.Uint256) error {
...
func dbFetchBlockIDByHash(dbTx database.Tx, hash *common.Uint256) (uint32, error) {
...
func dbFetchBlockHashBySerializedID(dbTx database.Tx, serializedID []byte) (*common.Uint256, error) {
...
func dbFetchBlockHashByID(dbTx database.Tx, id uint32) (*common.Uint256, error) {
	var serializedID [4]byte
	byteOrder.PutUint32(serializedID[:], id)
...
func dbPutTxIndexEntry(dbTx database.Tx, txHash *common.Uint256, serializedData []byte) error {
...
func dbFetchTxIndexEntry(dbTx database.Tx, txHash *common.Uint256) (*database.BlockRegion, error) {
	...
	region := database.BlockRegion{Hash: &common.Uint256{}}
...
func dbAddTxIndexEntries(dbTx database.Tx, block *types.Block, blockID uint32) error {
	...
	for i, tx := range block.Transactions {
		...
		hash := tx.Hash()
		err := dbPutTxIndexEntry(dbTx, &hash,
...
func dbRemoveTxIndexEntry(dbTx database.Tx, txHash *common.Uint256) error {
	txIndex := dbTx.Metadata().Bucket(txIndexKey)
	serializedData := txIndex.Get(txHash[:])
	if len(serializedData) == 0 {
...
func dbRemoveTxIndexEntries(dbTx database.Tx, block *types.Block) error {
	for _, tx := range block.Transactions {
		hash := tx.Hash()
		err := dbRemoveTxIndexEntry(dbTx, &hash)
		if err != nil {
			return err
		}
	}
...
func (idx *TxIndex) Init() error {
	...
	err := idx.db.View(func(dbTx database.Tx) error {
		...
		log.Infof("Forward scan (highest known %d, next unknown %d)",
		...
		for {
			...
			log.Infof("Binary scan (highest known %d, next "+
...
func (idx *TxIndex) ConnectBlock(dbTx database.Tx, block *types.Block) error {
	...
	hash := block.Hash()
	err := dbPutBlockIDIndexEntry(dbTx, &hash, newBlockID)
	if err != nil {
		return err
	}
...
func (idx *TxIndex) DisconnectBlock(dbTx database.Tx, block *types.Block) error {
	...
	hash := block.Hash()
	if err := dbRemoveBlockIDIndexEntry(dbTx, &hash); err != nil {
		return err
	}
	idx.curBlockID--
	return nil

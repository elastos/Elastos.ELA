...
type mruInventoryMap struct {
	invMtx  sync.Mutex
	invMap  map[msg.InvVect]*list.Element // nearly O(1) lookups
...
//
// This function is safe for concurrent access.
func (m *mruInventoryMap) Exists(iv *msg.InvVect) bool {
	m.invMtx.Lock()
...
//
// This function is safe for concurrent access.
func (m *mruInventoryMap) Add(iv *msg.InvVect) {
	m.invMtx.Lock()
	defer m.invMtx.Unlock()
...
//
// This function is safe for concurrent access.
func (m *mruInventoryMap) Delete(iv *msg.InvVect) {
	m.invMtx.Lock()
	if node, exists := m.invMap[*iv]; exists {
...
func newMruInventoryMap(limit uint) *mruInventoryMap {
	m := mruInventoryMap{
		invMap:  make(map[msg.InvVect]*list.Element),
		invList: list.New(),
		limit:   limit,
	}
	return &m

...
func (this *TXNPool) AppendToTxnPool(txn *tx.NodeTransaction) ErrCode {
	...
	if errCode := ledger.Validator.CheckTransactionSanity(txn); errCode != Success {
		log.Info("NodeTransaction verification failed", txn.Hash())
		return errCode
	}
	if errCode := ledger.Validator.CheckTransactionContext(txn, ledger.DefaultLedger); errCode != Success {

...
func TestState_ProcessTransaction(t *testing.T) {
	...
	for i, p := range producers {
		...
		state.ProcessBlock(mockBlock(uint32(i+1), tx), nil, 0)
	}
	...
	state.ProcessBlock(mockBlock(11, tx), nil, 0)
	...
	state.ProcessBlock(mockBlock(13, tx, tx2), nil, 0)
	...
	state.ProcessBlock(mockBlock(14, tx), nil, 0)
...
func TestState_ProcessBlock(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		state.ProcessBlock(mockBlock(uint32(i+1), txs...), nil, 0)
	}
	...
	state.ProcessBlock(mockBlock(11, txs...), nil, 0)
	for i := range txs {
	...
	state.ProcessBlock(mockBlock(13, txs...), nil, 0)
	for _, pk := range publicKeys {
		p := state.GetProducer(pk)
	...
	state.ProcessBlock(mockBlock(14, txs...), nil, 0)
	...
	state.ProcessBlock(mockBlock(15, txs...), nil, 0)
...
func TestState_ProcessIllegalBlockEvidence(t *testing.T) {
	...
	state.ProcessBlock(mockBlock(11), nil, 0)
...
func TestState_GetHistory(t *testing.T) {
	...
	state.ProcessBlock(mockBlock(15, tx), nil, 0)
...
func TestState_IsDPOSTransaction(t *testing.T) {
	...
	state.ProcessBlock(mockBlock(1, tx), nil, 0)
	for i := uint32(2); i < 10; i++ {
		state.ProcessBlock(mockBlock(i), nil, 0)
	}
	...
	state.ProcessBlock(mockBlock(10, tx), nil, 0)
...
func TestState_InactiveProducer_RecoverFromInactiveState(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		state.ProcessBlock(mockBlock(uint32(currentHeight)), nil, 0)
		if !assert.Equal(t, 1, len(state.GetInactiveProducers())) {
			t.FailNow()
		}
		currentHeight++
...
func TestState_InactiveProducer_DuringUpdateVersion(t *testing.T) {
	...
	state.ProcessBlock(mockBlock(uint32(currentHeight), tx), nil, 0)

...
func (t *CRCAppropriationTransaction) CheckTransactionOutput() error {
	...
	if !t.Outputs()[0].ProgramHash.IsEqual(*chainParams.CRConfiguration.CRExpensesAddressUint168) {
		return errors.New("new CRCAppropriation tx must have the first" +
			"output to CR expenses address")
	}
	if !t.Outputs()[1].ProgramHash.IsEqual(*chainParams.CRConfiguration.CRAssetsAddressUint168) {
...
func (t *CRCAppropriationTransaction) SpecialContextCheck() (result elaerr.ELAError, end bool) {
	...
	// Check output amount to CRExpensesAddressUint168:
	// (CRAssetsAddressUint168 + CRExpensesAddressUint168)*CRCAppropriatePercentage/100 -
	// CRExpensesAddressUint168 + CRCCommitteeUsedAmount
	//
	// Outputs has check in CheckTransactionOutput function:
	// first one to CRCommitteeAddress, second one to CRAssetsAddressUint168

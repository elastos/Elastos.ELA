...
import (
	...
	"github.com/elastos/Elastos.ELA/netsync"
	"github.com/elastos/Elastos.ELA/pact"
	"github.com/elastos/Elastos.ELA/peer"
...
const (
	...
	// the server.
	defaultServices = pact.SFNodeNetwork | pact.SFNodeBloom | pact.SFNodeOpen |
		pact.SFTxFilters
)
...
func (sp *serverPeer) enforceTxFilterFlag(cmd string) bool {
	if sp.server.services&pact.SFTxFilters != pact.SFTxFilters {
		// Disconnect the peer regardless of protocol version or banning
		// state.
		log.Debugf("%s sent an unsupported %s request -- "+
			"disconnecting", sp, cmd)

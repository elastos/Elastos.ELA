...
func (ap *AuxPow) Check(hashAuxBlock Uint256, chainId int) bool {
	...
	hashAuxBlockRev, _ := Uint256ParseFromBytes(hashAuxBlock.ToArrayReverse())

	auxRootHash := CheckMerkleBranch(hashAuxBlockRev, ap.AuxMerkleBranch, ap.AuxMerkleIndex)

	script := ap.ParCoinbaseTx.TxIn[0].SignatureScript
	scriptStr := hex.EncodeToString(script)

	auxRootHashStr := hex.EncodeToString(auxRootHash.ToArrayReverse())

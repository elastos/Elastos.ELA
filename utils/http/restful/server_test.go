...
func TestRestfulServer(t *testing.T) {
	...
	// Invalid url
	err = s.RegisterAction(http.MethodGet, "api/test/", func(htp.Params) (interface{}, error) {
		return nil, nil
	})
	...
	for _, method := range methods {
		err = s.RegisterAction(method, "/api/test", func(params htp.Params) (interface{}, error) {
	...
	err = s.RegisterAction(http.MethodGet, "/api/test/:param", func(params htp.Params) (interface{}, error) {
...
func TestServer_ServeHTTP(t *testing.T) {
	...
	for i, method := range methods {
		me := methods[i]
		err := s.RegisterAction(method, "/api/test/param/:param", func(params htp.Params) (interface{}, error) {
	...
	for i, method := range methods {
		me := methods[i]
		err := s.RegisterAction(method, "/api/test/params/:a/:b/:c", func(params htp.Params) (interface{}, error) {
	...
	for i, method := range methods {
		me := methods[i]
		err := s.RegisterAction(method, "/api/test/param/:a/param/:b", func(params htp.Params) (interface{}, error) {
	...
	for _, method := range methods {
		req, err := http.NewRequest(method, "http://localhost:20336/api/test", nil)
	...
	for _, method := range methods {
		req, err := http.NewRequest(method, "http://localhost:20336/api/test/param/param", nil)
		if !assert.NoError(t, err) {
			t.FailNow()
		}
		_, err = http.DefaultClient.Do(req)
	...
	for _, method := range methods {
		req, err := http.NewRequest(method, "http://localhost:20336/api/test/params/a/b/c", nil)
		if !assert.NoError(t, err) {
			t.FailNow()
		}
		_, err = http.DefaultClient.Do(req)
	...
	for _, method := range methods {
		req, err := http.NewRequest(method, "http://localhost:20336/api/test/param/a/param/b", nil)
		if !assert.NoError(t, err) {
			t.FailNow()
		}
		_, err = http.DefaultClient.Do(req)
	...
	// post request
	_, err = http.Post("http://localhost:20336/api/test", "text/plain",
	...
	// not found
	resp, err := http.Get("http://localhost:20336/api/notfound")
	if !assert.NoError(t, err) {
		t.FailNow()
	}
	...
	err = s.RegisterGetAction("/api/get", func(params htp.Params) (interface{}, error) {
		t.Logf("GET request from path %s", "/api/get")
		return nil, nil
	})
	assert.NoError(t, err)

	err = s.RegisterPutAction("/api/put", func(params htp.Params) (interface{}, error) {
		t.Logf("PUT request from path %s", "/api/put")
		return nil, nil
	})
	assert.NoError(t, err)

	err = s.RegisterPatchAction("/api/patch", func(params htp.Params) (interface{}, error) {
		t.Logf("PATCH request from path %s", "/api/patch")
		return nil, nil
	})
	assert.NoError(t, err)

	err = s.RegisterDeleteAction("/api/delete", func(params htp.Params) (interface{}, error) {
	...
	for _, method := range methods {
		req, err := http.NewRequest(method, fmt.Sprintf("http://localhost:20336/api/%s",
	...
	_, err = http.Post("http://localhost:20336/api/post", "text/plain",
		bytes.NewReader([]byte("data")))
	if !assert.NoError(t, err) {
		t.FailNow()
	}

...
var dutyState *state.DutyStateMock
...
func TestBlockVersionMain_GetNormalArbitersDesc(t *testing.T) {
	...
	producers, err := version.GetNormalArbitersDesc(5, cfg.Chain.GetState().GetInterfaceProducers())
...
func TestBlockVersionMain_AssignCoinbaseTxRewards(t *testing.T) {
	...
	dutyState.CurrentCandidates = candidates
	dutyState.CurrentArbitersPrograms = arbitratorHashes
	dutyState.CurrentCandidatesPrograms = candidateHashes
...
func TestBlockVersionMain_GetNextOnDutyArbiter(t *testing.T) {
	var currentArbiter []byte

	currentArbiter = version.GetNextOnDutyArbiter(0, 0)
	assert.Equal(t, arbitrators[0], currentArbiter)

	currentArbiter = version.GetNextOnDutyArbiter(1, 0)
	assert.Equal(t, arbitrators[1], currentArbiter)

	currentArbiter = version.GetNextOnDutyArbiter(2, 0)
	assert.Equal(t, arbitrators[2], currentArbiter)

	currentArbiter = version.GetNextOnDutyArbiter(3, 0)
	assert.Equal(t, arbitrators[3], currentArbiter)

	currentArbiter = version.GetNextOnDutyArbiter(4, 0)
	assert.Equal(t, arbitrators[4], currentArbiter)

	currentArbiter = version.GetNextOnDutyArbiter(5, 0)
	assert.Equal(t, arbitrators[0], currentArbiter)

	currentArbiter = version.GetNextOnDutyArbiter(0, 1)
	assert.Equal(t, arbitrators[1], currentArbiter)

	currentArbiter = version.GetNextOnDutyArbiter(0, 2)
	assert.Equal(t, arbitrators[2], currentArbiter)

	currentArbiter = version.GetNextOnDutyArbiter(0, 3)
	assert.Equal(t, arbitrators[3], currentArbiter)

	currentArbiter = version.GetNextOnDutyArbiter(0, 4)
	assert.Equal(t, arbitrators[4], currentArbiter)

	currentArbiter = version.GetNextOnDutyArbiter(0, 5)
	assert.Equal(t, arbitrators[0], currentArbiter)

	currentArbiter = version.GetNextOnDutyArbiter(0, 6)
	assert.Equal(t, arbitrators[1], currentArbiter)
}

...
func (m *MemPool) store(txid common.Uint256, history TransactionHistory) error {
	m.l.Lock()
	defer m.l.Unlock()
...
func (m *MemPool) GetMemPoolTx(address *common.Uint168) (ret []TransactionHistoryDisplay) {
	m.l.RLock()
	defer m.l.RUnlock()
	for k, v := range m.p {

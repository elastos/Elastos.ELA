...
//
//   Field           Type             Size
//   block hash      common.Uint256   common.UINT256SIZE
...
func dbPutIndexerTip(dbTx database.Tx, idxKey []byte, hash *common.Uint256, height int32) error {
	serialized := make([]byte, common.UINT256SIZE+4)
	copy(serialized, hash[:])
	byteOrder.PutUint32(serialized[common.UINT256SIZE:], uint32(height))
...
func dbFetchIndexerTip(dbTx database.Tx, idxKey []byte) (*common.Uint256, int32, error) {
	indexesBucket := dbTx.Metadata().Bucket(indexTipsBucketName)
	serialized := indexesBucket.Get(idxKey)
	if len(serialized) < common.UINT256SIZE+4 {
	...
	copy(hash[:], serialized[:common.UINT256SIZE])
	height := int32(byteOrder.Uint32(serialized[common.UINT256SIZE:]))
...
func dbIndexConnectBlock(dbTx database.Tx, indexer Indexer, block *types.Block) error {
	...
	if !curTipHash.IsEqual(block.Header.Previous) {
	...
	if err := indexer.ConnectBlock(dbTx, block); err != nil {
		return err
	}

	// Update the current index tip.
	hash := block.Hash()
	return dbPutIndexerTip(dbTx, idxKey, &hash, int32(block.Height))
}
...
func dbIndexDisconnectBlock(dbTx database.Tx, indexer Indexer, block *types.Block) error {
	...
	if err := indexer.DisconnectBlock(dbTx, block); err != nil {
		return err
	}
...
func (m *Manager) maybeCreateIndexes(dbTx database.Tx) error {
	...
	for _, indexer := range m.enabledIndexes {
		...
		// uninitialized index.
		err := dbPutIndexerTip(dbTx, idxKey, &common.Uint256{}, -1)
		if err != nil {
			return err
		}
	}
...
func (m *Manager) Init(chain IChain, interrupt <-chan struct{}) error {
	...
	for i := len(m.enabledIndexes); i > 0; i-- {
		...
		for !chain.MainChainHasBlock(uint32(height), hash) {
			...
			err := m.db.View(func(dbTx database.Tx) error {
				...
				blockBuf := bytes.NewBuffer(blockBytes)
				err = block.Deserialize(blockBuf)
				return err
			})
			if err != nil {
				return err
			...
			err = m.db.Update(func(dbTx database.Tx) error {
				...
				err = dbIndexDisconnectBlock(dbTx, indexer, &block)
				if err != nil {
					return err
				}
...
func (m *Manager) ConnectBlock(dbTx database.Tx, block *types.Block) error {
	...
	for _, index := range m.enabledIndexes {
		err := dbIndexConnectBlock(dbTx, index, block)
		if err != nil {
			return err
		}
	}
...
func (m *Manager) DisconnectBlock(dbTx database.Tx, block *types.Block) error {
	...
	for _, index := range m.enabledIndexes {
		err := dbIndexDisconnectBlock(dbTx, index, block)
		if err != nil {
			return err
		}
	}

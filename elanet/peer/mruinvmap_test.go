...
func TestMruInventoryMap(t *testing.T) {
	...
	// inventory code.
	numInvVects := 10
	invVects := make([]*msg.InvVect, 0, numInvVects)
	for i := 0; i < numInvVects; i++ {
		hash := &common.Uint256{byte(i)}
...
testLoop:
	for i, test := range tests {
		...
		if test.limit > 1 {
			...
			iv := msg.NewInvVect(msg.InvTypeBlock,
				&common.Uint256{0x00, 0x01})
			mruInvMap.Add(iv)
...
func TestMruInventoryMapStringer(t *testing.T) {
	...
	hash1 := &common.Uint256{0x01}
	hash2 := &common.Uint256{0x02}
	iv1 := msg.NewInvVect(msg.InvTypeBlock, hash1)
	iv2 := msg.NewInvVect(msg.InvTypeBlock, hash2)
...
func BenchmarkMruInventoryList(b *testing.B) {
	...
	invVects := make([]*msg.InvVect, 0, numInvVects)
	for i := 0; i < numInvVects; i++ {
		hashBytes := make([]byte, common.UINT256SIZE)
		rand.Read(hashBytes)

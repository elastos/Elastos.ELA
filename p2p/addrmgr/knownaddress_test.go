...
func TestChance(t *testing.T) {
	...
	var tests = []struct {
		...
		{
			//Test normal case
			addrmgr.TstNewKnownAddress(&p2p.NetAddress{Timestamp: now.Add(-35 * time.Second)},
				0, time.Now().Add(-30*time.Minute), time.Now(), false, 0),
			1.0,
		}, {
			//Test case in which lastseen < 0
			addrmgr.TstNewKnownAddress(&p2p.NetAddress{Timestamp: now.Add(20 * time.Second)},
				0, time.Now().Add(-30*time.Minute), time.Now(), false, 0),
			1.0,
		}, {
...
func TestIsBad(t *testing.T) {
	...
	futureNa := &p2p.NetAddress{Timestamp: future}
	minutesOldNa := &p2p.NetAddress{Timestamp: minutesOld}
	monthOldNa := &p2p.NetAddress{Timestamp: monthOld}
	currentNa := &p2p.NetAddress{Timestamp: secondsOld}

...
var (
	...
	AuxPowFactorySingleton AuxPowFactory
)

type AuxPowBase interface {
	Serialize(w io.Writer) error
	Deserialize(r io.Reader) error
	Check(hashAuxBlock Uint256, chainId int) bool
	GetBlockHeader() *BtcBlockHeader
}

type AuxPowFactory interface {
	Create() AuxPowBase
}

type AuxPowFactoryImpl struct {
}

func (factory *AuxPowFactoryImpl) Create() AuxPowBase {
	return &AuxPow{}
}
...
func (ap *AuxPow)  GetBlockHeader() *BtcBlockHeader {
	return &ap.ParBlockHeader
}
...
func init() {
	AuxPowFactorySingleton = &AuxPowFactoryImpl{}
}
...
func (c *ChainStoreExtend) processVote(block *Block, voteTxHolder *map[string]common.VoteCategory) error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func doProcessVote(block *Block, voteTxHolder *map[string]common.VoteCategory) error {
	for i := 0; i < len(block.Transactions); i++ {
		tx := block.Transactions[i]
		version := tx.Version()
		txid, err := ReverseHexString(tx.Hash().String())
		vt := common.VoteCategory(0x00)
		if err != nil {
			return err
		}
		if version == 0x09 {
...
func (c *ChainStoreExtend) persistTxHistory(blk *Block) error {
	...
	for _, block := range blocks {
		...
		txhs := make([]common.TransactionHistory, 0)
		for i := 0; i < len(txs); i++ {
			tx := txs[i]
			txid, err := ReverseHexString(tx.Hash().String())
			if err != nil {
			...
			for _, attr := range tx.Attributes() {
				if attr.Usage == common.Memo {
					memo = attr.Data
				}
			}

			if txType == common.CoinBase {
				var to []Uint168
				hold := make(map[Uint168]Fixed64)
				txhsCoinBase := make([]common.TransactionHistory, 0)
				for _, vout := range tx.Outputs() {
					if !ContainsU168(vout.ProgramHash, to) {
						to = append(to, vout.ProgramHash)
						txh := common.TransactionHistory{}
				...
				isCrossTx := false
				if txType == common.TransferCrossChainAsset {
					isCrossTx = true
				}
...
func (c *ChainStoreExtend) GetTxHistory(addr string, order string, timestamp uint64) interface{} {
	...
	var txhs interface{}
	if order == "desc" {
		txhs = make(common.TransactionHistorySorterDesc, 0)
	} else {
		txhs = make(common.TransactionHistorySorter, 0)
	}
	...
	for iter.Next() {
		...
		if (timestamp > 0 && txhd.Time > timestamp) || timestamp == 0 {
			if order == "desc" {
				txhs = append(txhs.(common.TransactionHistorySorterDesc), *txhd)
			} else {
				txhs = append(txhs.(common.TransactionHistorySorter), *txhd)
			}
		}
	}
	...
	for _, txh := range txInMempool {
		if order == "desc" {
			txhs = append(txhs.(common.TransactionHistorySorterDesc), txh)
		} else {
			txhs = append(txhs.(common.TransactionHistorySorter), txh)
		}
	}

	if order == "desc" {
		sort.Sort(txhs.(common.TransactionHistorySorterDesc))
	} else {
		sort.Sort(txhs.(common.TransactionHistorySorter))
	}
...
func (c *ChainStoreExtend) GetTxHistoryByLimit(addr, order string, skip, limit, timestamp uint32) (interface{}, int) {
	...
	if order == "desc" {
		return txhs.(common.TransactionHistorySorterDesc).Filter(skip, limit), len(txhs.(common.TransactionHistorySorterDesc))
	} else {
		return txhs.(common.TransactionHistorySorter).Filter(skip, limit), len(txhs.(common.TransactionHistorySorter))
	}
}
